...
async function getData() {
  ...
  try {
    const response = await axios.get(
      "https://www.timesjobs.com/candidate/job-search.html?searchType=personalizedSearch&from=submit&searchTextSrc=as&searchTextText=Noida%2F+Greater+Noida&txtKeywords=react+developer&txtLocation=Noida%2F+Greater+Noida&cboWorkExp1=0"
    );
    ...
    $(".clearfix>li>span").each((index, element) => {
    ...
    allJobDetails.forEach((job) => {
      ...
      ws.cell(rowIndex, 4).string(job.location);
      ws.cell(rowIndex, 5).string(job.postedDate);
      ws.cell(rowIndex, 6).string(job.jobDescription);
      rowIndex++;
    });
